...
### Test Upload (cURL)
```bash
curl -X POST https://safdal.investee-group.com/server/api/sync/agents/upload.php \
...
```bash
# All agents (admin)
curl "https://safdal.investee-group.com/server/api/sync/agents/changes.php?user_role=admin&limit=5"

# Filtered by shop (agent)
curl "https://safdal.investee-group.com/server/api/sync/agents/changes.php?shop_id=1&user_role=agent&limit=5"

# Incremental sync
curl "https://safdal.investee-group.com/server/api/sync/agents/changes.php?since=2025-12-10T10:00:00.000&user_role=admin"
```
