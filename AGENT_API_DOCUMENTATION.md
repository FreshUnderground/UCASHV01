...
### Test Upload (cURL)
```bash
curl -X POST http://localhost/UCASHV01/server/api/sync/agents/upload.php \
...
```bash
# All agents (admin)
curl "http://localhost/UCASHV01/server/api/sync/agents/changes.php?user_role=admin&limit=5"

# Filtered by shop (agent)
curl "http://localhost/UCASHV01/server/api/sync/agents/changes.php?shop_id=1&user_role=agent&limit=5"

# Incremental sync
curl "http://localhost/UCASHV01/server/api/sync/agents/changes.php?since=2025-12-10T10:00:00.000&user_role=admin"
```
