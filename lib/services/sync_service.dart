...
class SyncService {
  ...
  bool _isAutoSyncEnabled =
      true; // Activé par défaut pour synchronisation automatique
  ...
  static Duration get _slowSyncInterval =>
      SyncConfig.slowSyncInterval; // 10 minutes
  ...
  final List<Map<String, dynamic>> _pendingFlots =
      []; // File d'attente pour les flots
  ...
  Map<String, dynamic> _applyDelta(
      Map<String, dynamic> original, Map<String, dynamic> delta) {
    if (!SyncConfig.enableDeltaSync) {
  ...
  Future<List<Map<String, dynamic>>> _getLocalChangesWithDelta(
  ...
  void _addOperationToQueue(Map<String, dynamic> operation,
      {int priority = 1}) {
    ...
    debugPrint(
        '📋 Opération ajoutée à la queue (priorité: $priority): code_ops=${operation['code_ops']}');
  }
  ...
  void _cleanupOldPendingOperations() {
    ...
    _pendingSyncCount = _pendingOperations.length;
    debugPrint(
        '🧹 Nettoyage des anciennes opérations: ${_pendingOperations.length} restantes');
  }
  ...
  void _cleanupOldPendingFlots() {
    ...
    debugPrint(
        '🧹 Nettoyage des anciens flots: ${_pendingFlots.length} restants');
  }
  ...
  Future<void> initialize() async {
    ...
    final needsCommissionReset =
        !prefs.containsKey('commissions_cache_reset_v1');

    if (needsCommissionReset) {
      debugPrint(
          '🆕 Première utilisation après mise à jour - reset cache commissions nécessaire');
      try {
    ...
    if (_isAutoSyncEnabled) {
      startAutoSync();
      debugPrint(
          '⏰ Synchronisation automatique activée (intervalle: ${_autoSyncInterval.inSeconds}s)');

      // Démarrer aussi la sync spécialisée FLOTS & OPERATIONS
      startFlotsOpsAutoSync();
      debugPrint(
          '🚀⏰ Synchronisation auto FLOTS & OPERATIONS activée (intervalle: ${_autoSyncInterval.inSeconds}s)');

      // Démarrer la sync lente PERSONNEL (toutes les 10 minutes)
      startSlowSync();
      debugPrint(
          '🐢⏰ Synchronisation lente PERSONNEL activée (intervalle: ${_slowSyncInterval.inMinutes} min)');
    }

    debugPrint(
        '✅ Service de synchronisation initialisé (auto-sync: ${_isAutoSyncEnabled ? "ON" : "OFF"})');
  }
  ...
  void _onConnectivityChanged(ConnectivityResult result) async {
    ...
    debugPrint(
        '📡 Connectivité changée: $result (${_isOnline ? "Online" : "Offline"})');

    if (_isOnline && wasOffline) {
      // Passage de offline à online - synchroniser les données en attente
      debugPrint(
          '🔄 Retour en ligne détecté - synchronisation des données en attente...');
  ...
  void setAutoSync(bool enabled) {
    ...
    debugPrint(
        '🔄 Synchronisation automatique: ${enabled ? "activée" : "désactivée"}');
  }
  ...
  Future<SyncResult> syncAll({String? userId}) async {
    ...
    debugPrint(
        '🚀 === DÉBUT SYNCHRONISATION BIDIRECTIONNELLE (User: $userIdToUse - Role: $userRole) ===');
    ...
    try {
      ...
      final dependentTables = [
        ...
        'depot_clients',
        'triangular_debt_settlements'
      ];
      for (String table in dependentTables) {
        try {
          debugPrint('📤 Début upload $table...');
          await _uploadTableDataWithRetry(
              table, userIdToUse, userRole); // Pass user role
          ...
          if (table == 'agents') {
            ...
            debugPrint(
                '🔺 Règlements triangulaires uploadés - pas de rechargement nécessaire');
          }
        } catch (e) {
      ...
      await prefs.setString(
          'last_sync_global', DateTime.now().toIso8601String());
      _lastSyncTime = DateTime.now();

      debugPrint('✅ === SYNCHRONISATION TERMINÉE AVEC SUCCÈS ===');
      _updateStatus(SyncStatus.idle);
      return SyncResult(
          success: true, message: 'Synchronisation terminée avec succès');
    } catch (e, stackTrace) {
  ...
  Future<void> resetCommissionsCache() async {
    try {
      ...
      debugPrint(
          '✅ ${commissions.length} commissions rechargées depuis le serveur');

      // Afficher les détails des commissions pour vérification
      for (var c in commissions) {
        debugPrint(
            '   📊 ${c.description}: ${c.taux}% (Source: ${c.shopSourceId}, Dest: ${c.shopDestinationId})');
      }

      debugPrint('✅ Cache des commissions réinitialisé avec succès');
    } catch (e) {
      debugPrint(
          '❌ Erreur lors de la réinitialisation du cache des commissions: $e');
      rethrow;
    }
  }

  /// Upload avec retry logic pour échecs temporaires
  Future<void> _uploadTableDataWithRetry(
      String tableName, String userId, String userRole,
    ...
    for (int attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        debugPrint(
            '⚠️ Upload $tableName tentative $attempt/$maxRetries échouée: $e');
  ...
  Future<void> _uploadLocalChanges(String userId) async {
    ...
    debugPrint(
        '📤 Début de l\'upload des données locales (${tables.length} tables)');

    for (String table in tables) {
      try {
        debugPrint('📤 Upload $table...');
        await _uploadTableDataWithRetry(
            table, userId, 'admin'); // Utiliser version avec retry
        successCount++;
      } catch (e) {
        debugPrint('❌ Erreur upload $table: $e');
        errorCount++;
  ...
  Future<bool> _validateEntityData(
      String tableName, Map<String, dynamic> data) async {
    switch (tableName) {
      case 'agents':
        if (data['username'] == null || data['username'].toString().isEmpty) {
          debugPrint(
              '❌ Validation: username manquant pour agent ${data['id']}');
          return false;
        }
      ...
      case 'clients':
        ...
        if (shopId == null) {
          ...
          debugPrint(
              'ℹ️ Client ${data['nom']} (ID: ${data['id']}): client global sans shop (shop_id = null)');
          return true;
        }
        ...
        if (shopId <= 0) {
          debugPrint(
              '❌ Validation: shop_id invalide ($shopId) pour client ${data['id']}');
          return false;
        }
      ...
      case 'operations':
        ...
        if (data['type'] == null) {
          debugPrint(
              '❌ Validation: type manquant pour operation ${data['id']}');
          return false;
        }
        if (data['montant_net'] == null || data['montant_net'] <= 0) {
          debugPrint(
              '❌ Validation: montant_net invalide pour operation ${data['id']}');
          return false;
        }
        if (data['shop_source_id'] == null || data['shop_source_id'] <= 0) {
          debugPrint(
              '❌ Validation: shop_source_id manquant pour operation ${data['id']}');
          return false;
        }
        return true;
      ...
      case 'shops':
        if (data['designation'] == null ||
            data['designation'].toString().isEmpty) {
          debugPrint(
              '❌ Validation: designation manquant pour shop ${data['id']}');
          return false;
        }
        return true;
      ...
      case 'triangular_debt_settlements':
        ...
        if (data['reference'] == null || data['reference'].toString().isEmpty) {
          debugPrint(
              '❌ Validation: reference manquante pour triangular_debt_settlement ${data['id']}');
          return false;
        }
        if (data['shop_debtor_id'] == null || data['shop_debtor_id'] <= 0) {
          debugPrint(
              '❌ Validation: shop_debtor_id manquant pour triangular_debt_settlement ${data['id']}');
          return false;
        }
        if (data['shop_intermediary_id'] == null ||
            data['shop_intermediary_id'] <= 0) {
          debugPrint(
              '❌ Validation: shop_intermediary_id manquant pour triangular_debt_settlement ${data['id']}');
          return false;
        }
        if (data['shop_creditor_id'] == null || data['shop_creditor_id'] <= 0) {
          debugPrint(
              '❌ Validation: shop_creditor_id manquant pour triangular_debt_settlement ${data['id']}');
          return false;
        }
        if (data['montant'] == null || data['montant'] <= 0) {
          debugPrint(
              '❌ Validation: montant invalide pour triangular_debt_settlement ${data['id']}');
          return false;
        }
        if (data['date_reglement'] == null) {
          debugPrint(
              '❌ Validation: date_reglement manquante pour triangular_debt_settlement ${data['id']}');
          return false;
        }
        if (data['agent_id'] == null || data['agent_id'] < 0) {
          debugPrint(
              '❌ Validation: agent_id manquant pour triangular_debt_settlement ${data['id']}');
          return false;
        }
        ...
        if (debtorId == intermediaryId ||
            debtorId == creditorId ||
            intermediaryId == creditorId) {
          debugPrint(
              '❌ Validation: les trois shops doivent être différents pour triangular_debt_settlement ${data['id']} (debtor: $debtorId, intermediary: $intermediaryId, creditor: $creditorId)');
          return false;
        }
      ...
      case 'flots':
        ...
        if (data['shop_source_id'] == null || data['shop_source_id'] <= 0) {
          debugPrint(
              '❌ Validation: shop_source_id manquant pour flot ${data['id']}');
          return false;
        }
        if (data['shop_destination_id'] == null ||
            data['shop_destination_id'] <= 0) {
          debugPrint(
              '❌ Validation: shop_destination_id manquant pour flot ${data['id']}');
          return false;
        }
        if (data['agent_envoyeur_id'] == null ||
            data['agent_envoyeur_id'] <= 0) {
          debugPrint(
              '❌ Validation: agent_envoyeur_id manquant pour flot ${data['id']}');
          return false;
        }
        if (data['montant'] == null || data['montant'] <= 0) {
      ...
      case 'taux':
        ...
        if (data['devise_source'] == null ||
            data['devise_source'].toString().isEmpty) {
          debugPrint(
              '❌ Validation: devise_source manquante pour taux ${data['id']}');
          return false;
        }
        if (data['devise_cible'] == null ||
            data['devise_cible'].toString().isEmpty) {
          debugPrint(
              '❌ Validation: devise_cible manquante pour taux ${data['id']}');
          return false;
        }
      ...
      case 'commissions':
        ...
        if (data['type'] == null) {
          debugPrint(
              '❌ Validation: type manquant pour commission ${data['id']}');
          return false;
        }
        if (data['taux'] == null || data['taux'] < 0) {
          debugPrint(
              '❌ Validation: taux invalide pour commission ${data['id']}');
          return false;
        }
        return true;
      ...
      case 'document_headers':
        ...
        if (data['entreprise_nom'] == null ||
            data['entreprise_nom'].toString().isEmpty) {
          debugPrint(
              '❌ Validation: entreprise_nom manquant pour document_header ${data['id']}');
          return false;
        }
        return true;
      ...
      case 'cloture_caisse':
        ...
        if (data['shop_id'] == null || data['shop_id'] <= 0) {
          debugPrint(
              '❌ Validation: shop_id manquant pour cloture_caisse ${data['id']}');
          return false;
        }
        if (data['date_cloture'] == null) {
          debugPrint(
              '❌ Validation: date_cloture manquante pour cloture_caisse ${data['id']}');
          return false;
        }
        return true;
      ...
      case 'sims':
        ...
        if (data['shop_id'] == null || data['shop_id'] <= 0) {
          debugPrint(
              '❌ Validation: shop_id manquant ou invalide pour sim ${data['id']} (valeur: ${data['shop_id']})');
          return false;
        }
        // Additional validation for shop_id type
        if (data['shop_id'] is! int) {
          debugPrint(
              '❌ Validation: shop_id doit être un entier pour sim ${data['id']} (valeur: ${data['shop_id']}, type: ${data['shop_id'].runtimeType})');
          return false;
        }
        return true;
      ...
      case 'virtual_transactions':
        ...
        if (data['reference'] == null || data['reference'].toString().isEmpty) {
          debugPrint(
              '❌ Validation: reference manquante pour virtual_transaction ${data['id']}');
          return false;
        }
        if (data['montant_virtuel'] == null || data['montant_virtuel'] <= 0) {
          debugPrint(
              '❌ Validation: montant_virtuel invalide pour virtual_transaction ${data['id']}');
          return false;
        }
        if (data['montant_cash'] == null || data['montant_cash'] < 0) {
          debugPrint(
              '❌ Validation: montant_cash invalide pour virtual_transaction ${data['id']}');
          return false;
        }
        if (data['sim_numero'] == null ||
            data['sim_numero'].toString().isEmpty) {
          debugPrint(
              '❌ Validation: sim_numero manquant pour virtual_transaction ${data['id']}');
          return false;
        }
        if (data['shop_id'] == null || data['shop_id'] <= 0) {
          debugPrint(
              '❌ Validation: shop_id manquant ou invalide pour virtual_transaction ${data['id']}');
          return false;
        }
        if (data['agent_id'] == null || data['agent_id'] <= 0) {
          debugPrint(
              '❌ Validation: agent_id manquant ou invalide pour virtual_transaction ${data['id']}');
          return false;
        }
        return true;
      ...
      case 'sim_movements':
        ...
        if (data['sim_id'] == null || data['sim_id'] <= 0) {
          debugPrint(
              '❌ Validation: sim_id manquant ou invalide pour sim_movement ${data['id']}');
          return false;
        }
        if (data['sim_numero'] == null ||
            data['sim_numero'].toString().isEmpty) {
          debugPrint(
              '❌ Validation: sim_numero manquant pour sim_movement ${data['id']}');
          return false;
        }
        if (data['nouveau_shop_id'] == null || data['nouveau_shop_id'] <= 0) {
          debugPrint(
              '❌ Validation: nouveau_shop_id manquant ou invalide pour sim_movement ${data['id']}');
          return false;
        }
        if (data['nouveau_shop_designation'] == null ||
            data['nouveau_shop_designation'].toString().isEmpty) {
          debugPrint(
              '❌ Validation: nouveau_shop_designation manquant pour sim_movement ${data['id']}');
          return false;
        }
        if (data['admin_responsable'] == null ||
            data['admin_responsable'].toString().isEmpty) {
          debugPrint(
              '❌ Validation: admin_responsable manquant pour sim_movement ${data['id']}');
          return false;
        }
        return true;
      ...
      case 'comptes_speciaux':
        ...
        if (data['type'] == null || data['type'].toString().isEmpty) {
          debugPrint(
              '❌ Validation: type manquant pour compte_special ${data['id']}');
          return false;
        }
        if (data['type_transaction'] == null ||
            data['type_transaction'].toString().isEmpty) {
          debugPrint(
              '❌ Validation: type_transaction manquant pour compte_special ${data['id']}');
          return false;
        }
        ...
        if (!validTypes.contains(data['type'])) {
          debugPrint(
              '❌ Validation: type invalide "${data['type']}" pour compte_special ${data['id']} (valeurs acceptées: ${validTypes.join(", ")})');
          return false;
        }
        if (!validTransactionTypes.contains(data['type_transaction'])) {
          debugPrint(
              '❌ Validation: type_transaction invalide "${data['type_transaction']}" pour compte_special ${data['id']} (valeurs acceptées: ${validTransactionTypes.join(", ")})');
          return false;
        }
        ...
        if (montant == null) {
          debugPrint(
              '❌ Validation: montant null pour compte_special ${data['id']}');
          return false;
        }

        // Convertir en num pour la comparaison
        final montantNum =
            montant is num ? montant : num.tryParse(montant.toString());
        if (montantNum == null) {
          debugPrint(
              '❌ Validation: montant non numérique pour compte_special ${data['id']} (valeur: $montant)');
          return false;
        }
        ...
        if (typeTransaction == 'RETRAIT' || typeTransaction == 'SORTIE') {
          ...
          if (montantNum == 0) {
            debugPrint(
                '❌ Validation: montant zéro pour compte_special ${data['id']} (type: $typeTransaction)');
            return false;
          }
        } else {
          ...
          if (montantNum <= 0) {
            debugPrint(
                '❌ Validation: montant invalide ($montantNum) pour compte_special ${data['id']} (type: $typeTransaction, doit être > 0)');
            return false;
          }
        }
  ...
  Future<void> _uploadTableData(String tableName, String userId,
      [String userRole = 'admin']) async {
    try {
      ...
      if (tableName == 'triangular_debt_settlements') {
        debugPrint(
            '🔺 Upload règlements triangulaires: ${localData.length} éléments');
      }
      ...
      if (localData.isEmpty) {
        ...
        if (tableName == 'triangular_debt_settlements') {
          debugPrint(
              '🔺 Aucun règlement triangulaire à synchroniser - SUCCESS (rien à faire)');
        }
      ...
      for (var data in localData) {
        ...
        if (isValid) {
          ...
          debugPrint(
              '⚠️ $tableName: Données invalides pour ID ${data['id']} - ignorées');
        }
      }

      if (invalidData.isNotEmpty) {
        debugPrint(
            '⚠️ $tableName: ${invalidData.length} éléments invalides ignorés');
      }
      ...
      if (tableName == 'triangular_debt_settlements') {
        ...
        debugPrint(
            '🔺 Payload: ${jsonData.substring(0, jsonData.length > 500 ? 500 : jsonData.length)}...');
      }
      ...
      try {
        ...
        if (tableName == 'triangular_debt_settlements') {
          ...
          try {
            ...
            final pingResponse =
                await http.get(pingUrl).timeout(Duration(seconds: 10));
            debugPrint(
                '🔺 Ping serveur: ${pingResponse.statusCode} - ${pingResponse.body.substring(0, 100)}');
          } catch (pingError) {
          ...
          if (e.toString().contains('Connection') ||
            ...
            debugPrint(
                '🔺 Erreur de connectivité temporaire pour triangular - skip pour éviter circuit breaker');
      ...
      if (response.statusCode == 200) {
        ...
        if (!responseBody.startsWith('{') && !responseBody.startsWith('[')) {
          ...
          debugPrint(
              '📄 Contenu brut (premiers 1000 caractères): ${responseBody.substring(0, responseBody.length > 1000 ? 1000 : responseBody.length)}');
          throw FormatException(
              'La réponse du serveur n\'est pas du JSON valide pour $tableName: ${responseBody.substring(0, responseBody.length > 100 ? 100 : responseBody.length)}');
        }
        ...
        if (result['success'] == true) {
          ...
          if (errors.isNotEmpty) {
            for (var error in errors) {
              debugPrint(
                  '⚠️ Erreur $tableName ID ${error['entity_id']}: ${error['error']}');
            }
          }
          ...
          if (tableName == 'operations' && (uploaded > 0 || updated > 0)) {
            ...
            for (var data in localData) {
              if (data['destinataire'] == 'CAPITAL INITIAL') {
                ...
                debugPrint(
                    '💰 OP ${data['id']}: Opération de capital initial uploadée avec succès');
              }
            }
            if (initialCapitalUploaded > 0) {
              debugPrint(
                  '💰 $tableName: $initialCapitalUploaded opérations de capital initial uploadées');
            }
          }
          ...
          if (uploaded > 0 || updated > 0) {
            ...
            if (tableName == 'triangular_debt_settlements') {
              debugPrint(
                  '🔺 ${uploaded + updated} règlements triangulaires marqués comme synchronisés');
            }
          }
        } else {
  ...
  Future<void> uploadTableData(String tableName, String userId,
      [String userRole = 'admin']) async {
  ...
  Future<void> downloadTableData(
      String tableName, String userId, String userRole) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (result['success'] == true) {
          ...
          debugPrint(
              '✅ Admins synchronisés: ${stats['created']} créés, ${stats['updated']} mis à jour');
          ...
          if (errors.isNotEmpty) {
            for (var error in errors) {
              debugPrint(
                  '⚠️ Erreur admin ${error['username']}: ${error['error']}');
            }
          }
        } else {
          debugPrint(
              '⚠️ Erreur sync admins: ${result['error'] ?? result['message']}');
        }
      } else {
          ...
          for (var adminData in admins) {
            ...
            if (adminId != null && adminId > 0) {
              ...
              if (existingAdminData == null) {
                ...
                await prefs.setString(
                    'admin_$adminId', jsonEncode(serverAdmin));
                debugPrint(
                    '✅ Admin $adminId (${adminData['username']}) téléchargé et sauvegardé');
              } else {
                ...
                if (serverUpdatedAt != null && localCreatedAt != null) {
                  try {
                    final serverDate =
                        DateTime.parse(serverUpdatedAt.toString());
                    ...
                    if (serverDate.isAfter(localDate)) {
                      await prefs.setString(
                          'admin_$adminId', jsonEncode(serverAdmin));
                      debugPrint(
                          '🔄 Admin $adminId mis à jour depuis le serveur');
                    }
                  } catch (e) {
                    debugPrint(
                        '⚠️ Erreur comparaison dates admin $adminId: $e');
                  }
                }
              }
            }
  ...
  Future<Map<String, dynamic>> downloadAllComptesSpeciaux({
    ...
    try {
      ...
      if (userRole != 'admin' && shopId != null) {
        ...
        debugPrint(
            '💰 Mode AGENT: filtrage COMPTES SPÉCIAUX par shop_id=$shopId');
      } else {
        debugPrint(
            '👑 Mode ADMIN: téléchargement de TOUS les comptes spéciaux');
      }
      ...
      if (response.statusCode == 200) {
        ...
        if (result['success'] == true) {
          ...
          debugPrint(
              '✅ Comptes spéciaux téléchargés: ${entities.length} / $totalCount');
          debugPrint(
              '   📊 FRAIS: ${summary['nombre_frais']} transactions, total: \$${summary['total_frais']}');
          debugPrint(
              '   📊 DÉPENSE: ${summary['nombre_depense']} transactions, total: \$${summary['total_depense']}');
          ...
          return {
            ...
            'summary': summary,
            'message':
                'Téléchargement réussi: ${entities.length} comptes spéciaux',
          };
        } else {
          throw Exception('Erreur serveur: ${result['message']}');
        }
  ...
  Future<void> _downloadRemoteChanges(String userId, String userRole) async {
    ...
    debugPrint(
        '📥 Début du download des données distantes (${tables.length} tables)');
    debugPrint(
        '⚠️ depot_clients synchronisé par DepotRetraitSyncService (ignoré ici)');
    ...
    for (String table in tables) {
      try {
        ...
        if (table == 'triangular_debt_settlements') {
          debugPrint(
              '🔺 Règlements triangulaires téléchargés depuis le serveur');
        }
        successCount++;
      } catch (e) {
    ...
    debugPrint(
        '📥 Download terminé: $successCount succès, $errorCount erreurs');
  }

  /// Download des données d'une table spécifique
  Future<void> _downloadTableData(
      String tableName, String userId, String userRole) async {
    try {
      ...
      if (tableName == 'triangular_debt_settlements') {
        debugPrint(
            '🔺 Début download règlements triangulaires depuis: $lastSync');
      }
      ...
      if (tableName == 'virtual_transactions') {
        ...
        sinceParam =
            '2020-01-01T00:00:00.000'; // Date par défaut, le filtrage se fait côté serveur
        debugPrint(
            '💰 VIRTUAL_TRANSACTIONS: Utilisation du filtrage intelligent côté serveur');
        debugPrint(
            '   🎯 Filtrage: EN ATTENTE (toutes) + VALIDÉES (2j) + ANNULÉES (1j)');
      } else {
        ...
        if (lastSync != null) {
          ...
          debugPrint(
              '🔄 $tableName: Overlap window applied (60s before $lastSync)');
        }
      ...
      debugPrint(
          '📥 $tableName: Downloading since $sinceParam ${lastSync != null ? '(with 60s overlap)' : '(initial sync)'}');
      ...
      final prefs = await SharedPreferences.getInstance();
      final currentShopId =
          prefs.getInt('current_shop_id'); // Shop de l'utilisateur connecté
      ...
      if (tableName == 'agents') {
        ...
        uri = Uri.parse('$baseUrl/$tableName/$endpoint')
            .replace(queryParameters: queryParams);

        if (userRole == 'admin') {
          debugPrint(
              '👑 Mode ADMIN: téléchargement de toutes les données $tableName');
        } else {
          debugPrint(
              '👤 Mode AGENT: filtrage $tableName par shop_id=$currentShopId');
        }
        ...
        if (userRole != 'admin' && currentShopId != null) {
          queryParams['shop_id'] = currentShopId.toString();
          debugPrint(
              '🚚 Mode AGENT: filtrage FLOTs par shop_id=$currentShopId (source OU destination)');
        } else {
        ...
        debugPrint(
            '   🎯 Filtrage: EN ATTENTE (toutes) + SERVIS (4j) + ANNULÉS (1j)');
        ...
        debugPrint(
            '   🎯 Filtrage: EN ATTENTE (toutes) + VALIDÉES (4j) + ANNULÉES (1j)');
        ...
        if (userRole != 'admin' && currentShopId != null) {
          queryParams['shop_id'] = currentShopId.toString();
          debugPrint(
              '💰 Mode AGENT: filtrage COMPTES SPÉCIAUX par shop_id=$currentShopId');
        } else {
          debugPrint(
              '👑 Mode ADMIN: téléchargement de TOUS les comptes spéciaux');
        }
        ...
        final triangularSinceParam =
            lastSync == null ? '2020-01-01T00:00:00.000' : sinceParam;
        final queryParams = {
        ...
        if (lastSync == null) {
          debugPrint(
              '🔺 PREMIÈRE SYNC: Téléchargement de TOUS les règlements triangulaires depuis 2020');
        } else {
          debugPrint(
              '🔺 SYNC INCRÉMENTALE: Règlements triangulaires depuis $lastSync');
        }

        if (userRole != 'admin' && currentShopId != null) {
          queryParams['shop_id'] = currentShopId.toString();
          debugPrint(
              '🔺 Mode AGENT: filtrage RÈGLEMENTS TRIANGULAIRES par shop_id=$currentShopId');
        } else {
          debugPrint(
              '👑 Mode ADMIN: téléchargement de TOUS les règlements triangulaires');
        }
      ...
      if (response.statusCode == 200) {
        ...
        if (!responseBody.startsWith('{') && !responseBody.startsWith('[')) {
          ...
          debugPrint(
              '📄 Contenu brut (premiers 500 caractères): ${responseBody.substring(0, responseBody.length > 500 ? 500 : responseBody.length)}');
          throw FormatException(
              'La réponse du serveur n\'est pas du JSON valide pour $tableName');
        }
        ...
        if (result['success'] == true) {
          ...
          debugPrint(
              '📥 $tableName: ${remoteData.length} éléments reçus du serveur');
          ...
          if (tableName == 'sims') {
            if (remoteData.isEmpty) {
              ...
              debugPrint(
                  '   Vérifiez si des SIMs existent dans la base de données serveur');
              debugPrint('   URL requête: $uri');
            } else {
              debugPrint('📱 SIMs reçues du serveur:');
              for (var simData in remoteData) {
                debugPrint(
                    '   - ID: ${simData['id']}, Numéro: ${simData['numero']}, Opérateur: ${simData['operateur']}, Shop: ${simData['shop_id']}');
              }
            }
          }
          ...
          if (remoteData.isNotEmpty) {
            ...
            debugPrint(
                '🔄 Rechargement des données $tableName en mémoire après download...');
            switch (tableName) {
              case 'shops':
              ...
              case 'triangular_debt_settlements':
                debugPrint(
                    '🔺 Règlements triangulaires téléchargés et traités avec succès');
                debugPrint(
                    '🔺 Pas de rechargement en mémoire nécessaire pour les règlements triangulaires');
                break;
              case 'operations':
                ...
                if (userRole == 'admin') {
                  ...
                  await operationService.loadOperations();
                  debugPrint(
                      '👑 Admin: ${operationService.operations.length} opérations chargées (TOUTES)');
                  ...
                  debugPrint(
                      '👤 Agent: ${operationService.operations.length} opérations chargées (shop $currentShopId)');
                } else {
                  debugPrint(
                      '⚠️ Impossible de recharger les opérations: pas de contexte utilisateur');
                }
                break;
              case 'sims':
              ...
              case 'sim_movements':
                ...
                debugPrint(
                    '📝 Rechargement des mouvements de SIM en mémoire...');
                await SimService.instance.loadMovements();
                break;
              case 'virtual_transactions':
                // Recharger les transactions virtuelles dans le service
                debugPrint(
                    '💰 Rechargement des transactions virtuelles en mémoire...');
          ...
          final prefs = await SharedPreferences.getInstance();
          await prefs.setString(
              'last_sync_$tableName', DateTime.now().toIso8601String());
        ...
        debugPrint(
            '⚠️ Erreur HTTP $tableName: ${response.statusCode} - ${response.body}');
        throw Exception('Erreur HTTP ${response.statusCode}: ${response.body}');
      }
    } catch (e) {
  ...
  Future<void> _processRemoteChanges(
      String tableName, List remoteData, String userId) async {
    int updated = 0, inserted = 0, conflicts = 0, errors = 0;

    debugPrint(
        '🔄 Traitement de ${remoteData.length} éléments pour $tableName');
    ...
    if (tableName == 'sims') {
      ...
      debugPrint(
          '🗑️ Suppression de ${allLocalSims.length} SIMs locales existantes');
      ...
      for (var simData in remoteData) {
        try {
          ...
          inserted++;
          debugPrint(
              '  ✅ SIM ${sim.numero} insérée (Opérateur: ${sim.operateur}, Solde: ${sim.soldeActuel})');
        } catch (e) {
          errors++;
      ...
      await SimService.instance.loadSims();
      debugPrint(
          '✅ SIMs rechargées: ${SimService.instance.sims.length} SIMs disponibles');
    ...
    for (int i = 0; i < remoteData.length; i++) {
      try {
        ...
        debugPrint(
            '🔄 Traitement élément $i/${remoteData.length}: $tableName ID $entityId');
        ...
        if (localEntity == null) {
          ...
          final conflict = await _detectConflict(
              localEntity, remoteEntity, tableName, userId);

          if (conflict != null) {
            // Résoudre le conflit
            final resolved =
                await _resolveConflict(tableName, conflict, userId);
            if (resolved) {
              updated++;
            } else {
              conflicts++;
    ...
    debugPrint(
        '✅ $tableName: $inserted insérés, $updated mis à jour, $conflicts conflits, $errors erreurs');

    // CRITIQUE: Recharger les services en mémoire après traitement
    // NOTE: NE PAS utiliser clearBeforeLoad ici car les données sont déjà insérées dans LocalDB
    // On veut juste recharger en mémoire ce qui est déjà en base locale
    debugPrint(
        '🔄 Rechargement du service $tableName en mémoire après traitement...');
    switch (tableName) {
      case 'shops':
        await ShopService.instance.loadShops(forceRefresh: true);
        break;
      ...
      case 'operations':
        ...
        if (userRoleOps == 'admin') {
          ...
          debugPrint(
              '👑 Admin: ${operationServiceProcess.operations.length} opérations rechargées (TOUTES)');
        } else if (shopIdOps != null) {
          // Agent: charger seulement les opérations du shop
          await operationServiceProcess.loadOperations(shopId: shopIdOps);
          debugPrint(
              '👤 Agent: ${operationServiceProcess.operations.length} opérations rechargées (shop $shopIdOps)');
        }
        break;
      case 'sims':
  ...
  Future<ConflictInfo?> _detectConflict(Map<String, dynamic> local,
      Map<String, dynamic> remote, String tableName, String userId) async {
  ...
  Future<bool> _resolveConflict(
      String tableName, ConflictInfo conflict, String userId) async {
    debugPrint(
        '⚠️ Conflit détecté pour ${conflict.localData['id']} dans $tableName');
    ...
    switch (resolutionStrategy) {
      case ConflictResolutionStrategy.lastModifiedWins:
        resolvedSuccessfully =
            await _resolveWithLastModifiedWins(tableName, conflict);
        resolutionMethod = 'lastModifiedWins';
        break;

      case ConflictResolutionStrategy.mergeFields:
        resolvedSuccessfully =
            await _resolveWithFieldMerge(tableName, conflict);
      ...
      case ConflictResolutionStrategy.userChoice:
        ...
        resolvedSuccessfully =
            await _resolveWithUserChoice(tableName, conflict);
  ...
  Future<bool> _resolveWithLastModifiedWins(
      String tableName, ConflictInfo conflict) async {
    try {
      ...
      if (useRemote) {
        debugPrint(
            '🔄 Résolution: Utiliser la version distante (plus récente)');
  ...
  Future<bool> _resolveWithFieldMerge(
      String tableName, ConflictInfo conflict) async {
    try {
      ...
      final mergedData =
          _mergeEntityData(conflict.localData, conflict.remoteData);
  ...
  Future<bool> _resolveWithUserChoice(
      String tableName, ConflictInfo conflict) async {
      ...
      switch (tableName) {
        case 'shops':
          ...
          unsyncedData = shops
              .where((shop) {
                ...
                if (isNotSynced) {
                  debugPrint(
                      '📤 Shop "${shop.designation}" (ID ${shop.id}) à synchroniser (is_synced: ${shop.isSynced})');
                }
                return isNotSynced;
              })
          ...
          debugPrint(
              '📤 SHOPS: ${unsyncedData.length}/$totalShops non synchronisés');
        ...
        case 'agents':
          ...
          for (var shop in shops) {
            debugPrint(
                '   - Shop ID: ${shop.id}, Designation: "${shop.designation}"');
          }
          ...
          unsyncedData = agents
              .map((agent) {
                ...
                if (json['is_synced'] != true) {
                  debugPrint(
                      '🔄 Traitement agent ${agent.username} (ID ${json['id']}, Shop ID: ${agent.shopId})');
                  // ✅ Résoudre shop_designation depuis le shopId de l'agent
                  final agentShop =
                      shops.where((s) => s.id == agent.shopId).firstOrNull;
                  if (agentShop != null) {
                    json['shop_designation'] = agentShop.designation;
                    debugPrint(
                        '✅ Agent ${agent.username}: shopId=${agent.shopId} → shop_designation "${agentShop.designation}"');
                  } else {
                    debugPrint(
                        '⚠️ Agent ${agent.username}: shop ID ${agent.shopId} NON trouvé!');
                  }
                  return json;
                }
                return null;
          ...
          debugPrint(
              '👥 AGENTS: ${unsyncedData.length}/${totalAgents} non synchronisés (TOUS shops confondus)');
          ...
          if (unsyncedData.isEmpty && agents.isNotEmpty && since == null) {
            ...
            unsyncedData = agents.map((agent) {
              ...
              if (agentShop != null) {
                json['shop_designation'] = agentShop.designation;
                debugPrint(
                    '🔄 Agent ${agent.username} (ID ${json['id']}): shopId=${agent.shopId} → shop_designation "${agentShop.designation}"');
              }
              return json;
            }).toList();
          }
          ...
          unsyncedData = clients
              .map((client) {
                ...
                if (json['is_synced'] != true) {
                  ...
                  final clientShop =
                      shops.where((s) => s.id == client.shopId).firstOrNull;
                  if (clientShop != null) {
                    ...
                    debugPrint(
                        'ℹ️ Client ${client.nom}: shop_designation sera résolu côté serveur (shopId: ${client.shopId})');
                  }
        ...
        case 'triangular_debt_settlements':
          try {
            ...
            debugPrint(
                '🔺 TRIANGULAR: Total règlements en LocalDB: ${allSettlements.length}');
            ...
            for (var settlement in allSettlements) {
              try {
                ...
                if (json['is_synced'] != true) {
                  debugPrint(
                      '🔺 Règlement triangulaire non synchronisé: ${settlement.reference} (ID: ${settlement.id})');
                  unsyncedData.add(
                      _addSyncMetadata(json, 'triangular_debt_settlement'));
                }
              } catch (e) {
                debugPrint(
                    '⚠️ Erreur lors de la conversion JSON du règlement ${settlement.id}: $e');
                continue;
              }
            }

            debugPrint(
                '🔺 TRIANGULAR: ${unsyncedData.length}/${allSettlements.length} règlements à synchroniser');
          } catch (e) {
            debugPrint(
                '❌ Erreur lors de la récupération des règlements triangulaires: $e');
        ...
        case 'operations':
          ...
          debugPrint(
              '📦 OPERATIONS: Total opérations en mémoire: ${allOperations.length}');

          // Filtrer uniquement les opérations non synchronisées
          unsyncedData =
              allOperations.where((op) => op.isSynced != true).map((op) {
            final json = _addSyncMetadata(op.toJson(), 'operation');
            debugPrint(
                '📤 Opération ID ${op.id} à synchroniser: ${op.type.name} - ${op.montantNet} ${op.devise}');
            return json;
          }).toList();

          debugPrint(
              '📤 OPERATIONS: ${unsyncedData.length}/${allOperations.length} non synchronisées');

          // Si aucune opération non synchronisée mais qu'il y a des opérations, forcer l'upload en première sync
          if (unsyncedData.isEmpty &&
              allOperations.isNotEmpty &&
              since == null) {
            debugPrint(
                '🔄 Première synchronisation: envoi de toutes les opérations');
            unsyncedData = allOperations
                .map((op) => _addSyncMetadata(op.toJson(), 'operation'))
                .toList();
          }
          break;
        ...
        case 'commissions':
          ...
          debugPrint(
              '📤 COMMISSIONS: ${unsyncedData.length}/${commissions.length} non synchronisées');
          break;

        case 'comptes_speciaux':
          final transactions = CompteSpecialService.instance.transactions;
          debugPrint(
              '💰 COMPTES_SPECIAUX: Total en mémoire: ${transactions.length}');
          ...
          unsyncedData = transactions
              .map((t) {
                ...
                if (json['is_synced'] != true) {
                  debugPrint(
                      '📤 Compte spécial ID ${t.id} à synchroniser: ${t.type.name} - ${t.typeTransaction.name}');
                  return json;
                }
                return null;
              })
          ...
          debugPrint(
              '📤 COMPTES_SPECIAUX: ${unsyncedData.length}/${transactions.length} non synchronisés');
          break;
        ...
        case 'cloture_caisse':
          ...
          final prefs = await LocalDB.instance.database;
          final clotureKeys =
              prefs.getKeys().where((key) => key.startsWith('cloture_caisse_'));
          unsyncedData = [];
        ...
        case 'flots':
          ...
          unsyncedData =
              allFlots.where((flot) => flot.isSynced != true).map((flot) {
            final json = _addSyncMetadata(flot.toJson(), 'flot');
            debugPrint(
                '📤 Flot ID ${flot.id} à synchroniser: ${flot.shopSourceDesignation} → ${flot.shopDestinationDesignation} - ${flot.montant} ${flot.devise}');
            return json;
          }).toList();

          debugPrint(
              '📤 FLOTS: ${unsyncedData.length}/${allFlots.length} non synchronisés');
          ...
          if (unsyncedData.isEmpty && allFlots.isNotEmpty && since == null) {
            ...
            unsyncedData = allFlots
                .map((flot) => _addSyncMetadata(flot.toJson(), 'flot'))
                .toList();
          }
          break;
        ...
        case 'sims':
          ...
          if (allSims.isEmpty) {
            ...
            debugPrint(
                '   Vérifiez si des SIMs ont été créées dans l\'application');
          } else {
            debugPrint('📋 Liste des SIMs trouvées:');
            for (var sim in allSims) {
              debugPrint(
                  '   - SIM ID: ${sim.id}, Numéro: ${sim.numero}, Opérateur: ${sim.operateur}, isSynced: ${sim.isSynced}, Shop: ${sim.shopId}');
            }
          }

          // Filtrer uniquement les SIMs non synchronisées
          final simsToSync =
              allSims.where((sim) => sim.isSynced != true).toList();

          debugPrint(
              '📤 SIMS: ${simsToSync.length}/${allSims.length} non synchronisées');
          ...
          if (simsToSync.isEmpty && allSims.isNotEmpty) {
            ...
            for (var sim in simsToSync) {
              debugPrint(
                  '   → ${sim.numero} (${sim.operateur}) - Solde: ${sim.soldeActuel}');
            }
          }

          unsyncedData = simsToSync.map((sim) {
        ...
        case 'virtual_transactions':
          ...
          final allVirtualTransactions =
              await LocalDB.instance.getAllVirtualTransactions();
          debugPrint(
              '💰 VIRTUAL_TRANSACTIONS: Total en mémoire: ${allVirtualTransactions.length}');
          ...
          unsyncedData = allVirtualTransactions
              ...
              .map((transaction) {
            final json =
                _addSyncMetadata(transaction.toJson(), 'virtual_transaction');
            debugPrint(
                '📤 Virtual Transaction ${transaction.reference} à synchroniser: ${transaction.simNumero} - ${transaction.montantVirtuel} ${transaction.devise}');
            return json;
          }).toList();

          debugPrint(
              '📤 VIRTUAL_TRANSACTIONS: ${unsyncedData.length}/${allVirtualTransactions.length} non synchronisées');
          break;
        ...
        case 'depot_clients':
          ...
          unsyncedData =
              allDepots.where((depot) => depot.isSynced != true).map((depot) {
            final json = _addSyncMetadata(depot.toMap(), 'depot_client');
            debugPrint(
                '📤 Dépôt Client ID ${depot.id} à synchroniser: SIM ${depot.simNumero} - ${depot.montant} pour ${depot.telephoneClient}');
            return json;
          }).toList();

          debugPrint(
              '📤 DEPOT_CLIENTS: ${unsyncedData.length}/${allDepots.length} non synchronisés');
          break;
        ...
        case 'retrait_virtuels':
          ...
          debugPrint(
              '🔄 RETRAIT_VIRTUELS: Total en mémoire: ${allRetraits.length}');
          ...
          unsyncedData = allRetraits
            ...
            debugPrint(
                '📤 Retrait Virtuel ID ${retrait.id} à synchroniser: SIM ${retrait.simNumero} - ${retrait.montant} ${retrait.devise} (${retrait.statut.name})');
            return json;
          }).toList();

          debugPrint(
              '📤 RETRAIT_VIRTUELS: ${unsyncedData.length}/${allRetraits.length} non synchronisés');
          break;
        ...
        case 'audit_log':
          ...
          final prefs = await SharedPreferences.getInstance();
          final auditKeys =
              prefs.getKeys().where((key) => key.startsWith('audit_'));
          unsyncedData = [];
          ...
          debugPrint(
              '📤 AUDIT_LOG: ${unsyncedData.length} audits à synchroniser');
          break;
        ...
        case 'reconciliations':
          ...
          final prefs = await SharedPreferences.getInstance();
          final reconKeys =
              prefs.getKeys().where((key) => key.startsWith('reconciliation_'));
          unsyncedData = [];
          ...
          debugPrint(
              '📤 RECONCILIATIONS: ${unsyncedData.length} réconciliations à synchroniser');
          break;
        ...
        case 'credit_virtuels':
          ...
          debugPrint(
              '💳 CREDIT_VIRTUELS: Total en mémoire: ${allCredits.length}');
          ...
          unsyncedData = allCredits
            ...
            debugPrint(
                '📤 Crédit Virtuel ${credit.reference} à synchroniser: ${credit.beneficiaireNom} - ${credit.montantCredit} ${credit.devise} (${credit.statut.name})');
            return json;
          }).toList();

          debugPrint(
              '📤 CREDIT_VIRTUELS: ${unsyncedData.length}/${allCredits.length} non synchronisés');
          break;

        case 'credit_intershop_tracking':
          // Récupérer tous les trackings inter-shop depuis LocalDB
          final allTrackings =
              await LocalDB.instance.getAllCreditIntershopTracking();
          debugPrint(
              '🏦 CREDIT_INTERSHOP_TRACKING: Total en mémoire: ${allTrackings.length}');

          // Filtrer uniquement les trackings non synchronisés
          unsyncedData = allTrackings
              .where((tracking) => tracking.isSynced != true)
              .map((tracking) {
            final json = _addSyncMetadata(
                tracking.toJson(), 'credit_intershop_tracking');
            debugPrint(
                '📤 Tracking ID ${tracking.id}: ${tracking.shopNormalDesignation} → ${tracking.shopPrincipalDesignation} - ${tracking.montantBrut} ${tracking.devise}');
            return json;
          }).toList();

          debugPrint(
              '📤 CREDIT_INTERSHOP_TRACKING: ${unsyncedData.length}/${allTrackings.length} non synchronisés');
          break;

        default:
          debugPrint(
              '⚠️ Table inconnue pour récupération des changements: $tableName');
          return [];
      }

      if (unsyncedData.isNotEmpty) {
        debugPrint(
            '📤 $tableName: ${unsyncedData.length} enregistrement(s) non synchronisé(s) trouvé(s)');
      }
  ...
  Future<Map<String, dynamic>?> _getLocalEntity(
    ...
    try {
      final id =
          entityId is int ? entityId : int.tryParse(entityId.toString()) ?? 0;
      ...
      debugPrint(
          '❌ Erreur récupération entité locale $tableName/$entityId: $e');
      return null;
    }
  }

  /// Insère une nouvelle entité locale
  Future<void> _insertLocalEntity(
      String tableName, Map<String, dynamic> data) async {
    try {
      ...
      switch (tableName) {
        case 'shops':
          ...
          final existingShop =
              shops.where((s) => s.designation == designation).firstOrNull;
          if (existingShop != null && designation.isNotEmpty) {
            debugPrint(
                '⚠️ Doublon ignoré: shop designation "$designation" existe déjà');
            return;
          }
        ...
        case 'agents':
          ...
          final existingAgent =
              agents.where((a) => a.username == username).firstOrNull;
          if (existingAgent != null && username.isNotEmpty) {
            debugPrint(
                '⚠️ Doublon ignoré: agent username "$username" existe déjà');
            return;
          }
          ...
          if (shopDesignation != null && shopDesignation.isNotEmpty) {
            final shops = ShopService.instance.shops;
            final shop = shops
                .where((s) => s.designation == shopDesignation)
                .firstOrNull;
            if (shop != null) {
              shopId = shop.id!;
              debugPrint(
                  '🔍 Agent: shop_designation "$shopDesignation" → shop_id $shopId');
            } else {
              debugPrint(
                  '⚠️ Shop "$shopDesignation" NON trouvé pour agent "$username"!');
          ...
          debugPrint(
              '📥 Insertion agent depuis MySQL: ID=${agent.id}, username=${agent.username}, shopId=$shopId, shopDesignation=$shopDesignation');
        ...
        case 'clients':
          ...
          final existingClient =
              clients.where((c) => c.telephone == telephone).firstOrNull;
          if (existingClient != null && telephone.isNotEmpty) {
            debugPrint(
                '⚠️ Doublon ignoré: client téléphone "$telephone" existe déjà');
            return;
          }
          ...
          if (shopDesignation != null && shopDesignation.isNotEmpty) {
            ...
            if (shop != null) {
              shopId = shop.id!;
              debugPrint(
                  '🔍 Client: shop_designation "$shopDesignation" → shop_id $shopId');
            } else {
          ...
          if (agentUsername != null && agentUsername.isNotEmpty) {
            final agents = AgentService.instance.agents;
            final agent =
                agents.where((a) => a.username == agentUsername).firstOrNull;
            if (agent != null) {
              agentId = agent.id!;
              debugPrint(
                  '🔍 Client: agent_username "$agentUsername" → agent_id $agentId');
            } else {
          ...
          debugPrint(
              '🔍 Client download - ID: ${client.id}, shopId: $shopId, agentId: $agentId, nom: ${data['nom']}');
        ...
        case 'taux':
          ...
          if (existingTaux != null) {
            debugPrint(
                '⚠️ Doublon ignoré: taux ${taux.deviseSource}->${taux.deviseCible} (${taux.type}) existe déjà');
            return;
          }
        ...
        case 'commissions':
          ...
          final commissions = RatesService.instance.commissions;
          final existingById =
              commissions.where((c) => c.id == commission.id).firstOrNull;
          if (existingById != null) {
            debugPrint(
                '⚠️ Doublon ignoré: commission ID ${commission.id} existe déjà');
            return;
          }

          // Vérifier doublon par type + shopId + shopSourceId + shopDestinationId
          final existingByRoute = commissions
              .where((c) =>
          ...
          if (existingByRoute != null) {
            debugPrint(
                '⚠️ Doublon ignoré: commission similaire existe déjà (route identique)');
            return;
          }
        ...
        case 'comptes_speciaux':
          ...
          if (prefs.containsKey(existingKey)) {
            debugPrint(
                '⚠️ Doublon ignoré: compte spécial ID ${transaction.id} existe déjà');
            return;
          }
          ...
          debugPrint(
              '✅ Compte spécial ID ${transaction.id} sauvegardé: ${transaction.type.name} - \$${transaction.montant}');
        ...
        case 'document_headers':
          ...
          final oldHeaderKeys = allKeys.where(
              (key) => key.startsWith('document_header_') && key != activeKey);
          ...
          debugPrint(
              '✅ Document header ID ${header.id} sauvegardé dans $activeKey');
        ...
        case 'cloture_caisse':
          ...
          if (shopId != null && dateCloture != null) {
            ...
            final existingCloture = await LocalDB.instance
                .getClotureCaisseByDate(shopId, dateClotureObj);

            if (existingCloture != null) {
              debugPrint(
                  '⚠️ Doublon ignoré: clôture pour shop $shopId du ${dateClotureObj.toIso8601String().split('T')[0]} existe déjà (ID: ${existingCloture.id})');
              return;
            }
          }
          ...
          debugPrint(
              '✅ Clôture caisse shop ${cloture.shopId} du ${cloture.dateCloture.toIso8601String().split('T')[0]} sauvegardée (ID: ${cloture.id})');
          break;
        ...
        case 'flots':
          ...
          String? shopDestinationDesignation =
              data['shop_destination_designation'];
          ...
          if (shopSourceDesignation == null || shopSourceDesignation.isEmpty) {
            ...
            if (shopSourceId != null) {
              ...
              if (shop != null) {
                ...
                debugPrint(
                    '🔍 Flot: shop_source_id $shopSourceId → shop_source_designation "$shopSourceDesignation"');
              } else {
          ...
          if (shopDestinationDesignation == null ||
              shopDestinationDesignation.isEmpty) {
            final shopDestinationId = data['shop_destination_id'];
            if (shopDestinationId != null) {
              final shops = ShopService.instance.shops;
              final shop =
                  shops.where((s) => s.id == shopDestinationId).firstOrNull;
              if (shop != null) {
                shopDestinationDesignation = shop.designation;
                debugPrint(
                    '🔍 Flot: shop_destination_id $shopDestinationId → shop_destination_designation "$shopDestinationDesignation"');
              } else {
                debugPrint(
                    '⚠️ Shop destination ID $shopDestinationId non trouvé');
              }
            }
          }
          ...
          debugPrint(
              '⚠️ Flot ID ${flot.id} ignoré - Les FLOTs sont maintenant synchronisés via table operations');
        ...
        case 'operations':
          ...
          debugPrint(
              '✅ Opération ID ${operation.id} sauvegardée: ${operation.type.name} - ${operation.montantNet} ${operation.devise}');
          break;
        ...
        case 'sims':
          ...
          await LocalDB.instance.saveSim(sim);
          debugPrint(
              '✅ SIM ID ${sim.id} sauvegardée: ${sim.numero} - ${sim.operateur} - Solde: ${sim.soldeActuel}');
          break;
        ...
        case 'virtual_transactions':
          ...
          if (vtId != null) {
            final existingVt =
                await LocalDB.instance.getVirtualTransactionById(vtId);
            if (existingVt != null) {
              debugPrint(
                  '⚠️ Doublon ignoré: Transaction virtuelle ID $vtId existe déjà');
              return;
            }
          }
          ...
          debugPrint(
              '✅ Transaction virtuelle ID ${vt.id} sauvegardée: ${vt.reference} - ${vt.simNumero} - ${vt.montantVirtuel} ${vt.devise}');
          break;
        ...
        case 'triangular_debt_settlements':
          ...
          if (settlementId != null) {
            final existingSettlement = await LocalDB.instance
                .getTriangularDebtSettlementById(settlementId);
            if (existingSettlement != null) {
              debugPrint(
                  '⚠️ Doublon ignoré: Règlement triangulaire ID $settlementId existe déjà');
              return;
            }
          }
          ...
          debugPrint(
              '🔺 Règlement triangulaire ID ${settlement.id} sauvegardé: ${settlement.reference} - ${settlement.montant} ${settlement.devise}');
          break;
    ...
    try {
      ...
      switch (operation.type) {
        ...
        case OperationType.transfertInternationalSortant:
          libelle =
              'Transfert ${operation.type.name} - ${operation.destinataire} (Total reçu)';
          montant =
              operation.montantBrut; // TOTAL = montant à servir + commission
          type = TypeMouvement.entree; // ENTRÉE en caisse
          break;

        case OperationType.transfertInternationalEntrant:
          libelle =
              'Réception ${operation.type.name} - ${operation.destinataire}';
      ...
      debugPrint(
          '📋 Journal: ${type.name.toUpperCase()} de $montant ${operation.devise} - $libelle');
    } catch (e) {
    ...
    try {
      switch (tableName) {
        ...
        case 'comptes_speciaux':
          final transaction = CompteSpecialModel.fromJson(data);
          final prefs = await SharedPreferences.getInstance();
          await prefs.setString('compte_special_${transaction.id}',
        ...
        case 'document_headers':
          ...
          await prefs.setString(activeKey, jsonEncode(header.toJson()));
          debugPrint(
              '✅ Document header ID ${header.id} mis à jour dans $activeKey');
        ...
        case 'cloture_caisse':
          ...
          final prefs = await LocalDB.instance.database;
          final clotureKey =
              'cloture_caisse_${cloture.shopId}_${cloture.dateCloture.toIso8601String().split('T')[0]}';
          ...
          if (shopSourceDesignation == null || shopSourceDesignation.isEmpty) {
            ...
            if (shopSourceId != null) {
              ...
              if (shop != null) {
                shopSourceDesignation = shop.designation;
                debugPrint(
                    '🔍 Flot UPDATE: shop_source_id $shopSourceId → shop_source_designation "$shopSourceDesignation"');
              } else {
                debugPrint('⚠️ Shop source ID $shopSourceId non trouvé');
              }
            }
          ...
          if (shopDestinationDesignation == null ||
            ...
            if (shopDestinationId != null) {
              ...
              if (shop != null) {
                shopDestinationDesignation = shop.designation;
                debugPrint(
                    '🔍 Flot UPDATE: shop_destination_id $shopDestinationId → shop_destination_designation "$shopDestinationDesignation"');
              } else {
                debugPrint(
                    '⚠️ Shop destination ID $shopDestinationId non trouvé');
              }
        ...
        case 'sims':
          ...
          debugPrint(
              '✅ SIM ID ${sim.id} mise à jour: ${sim.numero} - ${sim.operateur} - Solde: ${sim.soldeActuel}');
          break;
        ...
        case 'virtual_transactions':
          ...
          debugPrint(
              '✅ Transaction virtuelle ID ${vt.id} mise à jour: ${vt.reference} - ${vt.simNumero}');
          break;
        ...
        case 'triangular_debt_settlements':
          ...
          debugPrint(
              '🔺 Règlement triangulaire ID ${settlement.id} mis à jour: ${settlement.reference} - ${settlement.montant} ${settlement.devise}');
          break;
  ...
  Future<void> _markEntitiesAsSynced(
    ...
    try {
        ...
        switch (tableName) {
          case 'shops':
            ...
            if (shopData != null) {
              ...
              debugPrint(
                  '✅ Shop ID $entityId marqué comme synchronisé dans LocalDB');

              // Mettre à jour également le cache en mémoire de ShopService
              final shop = ShopService.instance.getShopById(entityId);
              if (shop != null) {
                final index = ShopService.instance.shops
                    .indexWhere((s) => s.id == entityId);
                if (index != -1) {
                  ...
                  debugPrint(
                      '✅ Shop ID $entityId mis à jour dans le cache mémoire');
                }
              }
            } else {
              debugPrint(
                  '⚠️ Shop ID $entityId non trouvé dans LocalDB pour marquage sync');
            }
            break;
          ...
          case 'commissions':
            ...
            if (commissionData != null) {
              ...
              await prefs.setString(
                  'commission_$entityId', jsonEncode(commissionJson));
            }
            // Recharger les commissions en mémoire
            await RatesService.instance.loadRatesAndCommissions();
            break;
          ...
          case 'comptes_speciaux':
            ...
            if (transactionData != null) {
              ...
              await prefs.setString(
                  'compte_special_$entityId', jsonEncode(transactionJson));
            }
          ...
          case 'cloture_caisse':
            ...
            if (clotureData != null) {
              ...
              debugPrint(
                  '⚠️ Clôture ID $entityId non trouvée pour marquage sync (clé: $clotureKey)');
            }
            break;
          ...
          case 'virtual_transactions':
            ...
            if (vtData != null) {
              ...
              await prefs.setString(
                  'virtual_transaction_$entityId', jsonEncode(vtJson));
            }
            break;
          ...
          case 'triangular_debt_settlements':
            ...
            final settlementData =
                prefs.getString('triangular_debt_settlement_$entityId');
            if (settlementData != null) {
              ...
              await prefs.setString('triangular_debt_settlement_$entityId',
                  jsonEncode(settlementJson));
              debugPrint(
                  '🔺 Règlement triangulaire ID $entityId marqué comme synchronisé');
            } else {
              debugPrint(
                  '⚠️ Règlement triangulaire ID $entityId non trouvé pour marquage sync');
            }
            break;
        }
      }
      ...
      debugPrint(
          '✅ $tableName: ${entities.length} entités marquées comme synchronisées');
    } catch (e) {
      debugPrint(
          '❌ Erreur lors du marquage des entités comme synchronisées: $e');
    }
  }

  /// Marque une entité pour re-upload
  Future<void> _markEntityForReupload(
      String tableName, dynamic entityId) async {
  ...
  Future<bool> _checkConnectivity() async {
    try {
      ...
      debugPrint(
          '🌐 Test de connexion au serveur: $baseUrl/ping (essai avec .php)');
      ...
      if (isConnected) {
        ...
        debugPrint(
            '⚠️ Serveur inaccessible via $usedUrl (code ${response.statusCode}): ${response.body}');

        // Fournir des instructions de dépannage
        if (usedUrl.contains('localhost')) {
          debugPrint(
              '💡 Conseil: Vérifiez que Laragon est démarré avec Apache et MySQL');
          debugPrint(
              '💡 Conseil: Vérifiez que le chemin $usedUrl est accessible dans votre navigateur');
        }
      }
      ...
      if (e.toString().contains('XMLHttpRequest error')) {
        ...
        debugPrint(
            '💡 Solution: Vérifiez que l\'URL du serveur est correcte dans les paramètres');
        debugPrint(
            '💡 Solution: Vérifiez que les en-têtes CORS sont correctement configurés');
        debugPrint(
            '💡 Solution: Essayez d\'accéder à \$baseUrl/ping.php directement dans votre navigateur');
        ...
        debugPrint(
            '💡 Solution: Vérifiez que le serveur répond dans les temps');
      }
  ...
  Future<void> resetSyncStatus() async {
    ...
    debugPrint(
        '✅ Statut de synchronisation réinitialisé pour ${tables.length} tables');
  }
  ...
  void startAutoSync() {
    ...
    debugPrint(
        '⏰ Démarrage de la synchronisation automatique (intervalle: ${_autoSyncInterval.inSeconds}s)');
    debugPrint(
        '🔍 État: isAutoSyncEnabled=$_isAutoSyncEnabled, isOnline=$_isOnline, isSyncing=$_isSyncing');
    ...
    _autoSyncTimer = Timer.periodic(_autoSyncInterval, (timer) async {
      ...
      if (_isAutoSyncEnabled && !_isSyncing) {
        debugPrint(
            '🔄 [🕒 ${DateTime.now().toIso8601String()}] Synchronisation automatique - OPERATIONS, FLOTS, CLÔTURES & COMMISSIONS');
        ...
        _lastSyncTime = DateTime.now();
        debugPrint(
            '✅ Synchronisation automatique terminée: $successCount réussies, $errorCount échouées');
      } else {
        debugPrint(
            '⏸️ Synchronisation automatique ignorée (conditions non remplies)');
      }
    });
  ...
  void startFlotsOpsAutoSync() {
    ...
    debugPrint(
        '🚀⏰ Démarrage synchronisation auto FLOTS & OPERATIONS (intervalle: ${_autoSyncInterval.inSeconds}s)');
    debugPrint(
        '🔍 État: isAutoSyncEnabled=$_isAutoSyncEnabled, isOnline=$_isOnline, isSyncing=$_isSyncing');
    ...
    _flotsOpsAutoSyncTimer = Timer.periodic(_autoSyncInterval, (timer) async {
      ...
      if (_isAutoSyncEnabled && !_isSyncing && _isOnline) {
        debugPrint(
            '🔄 [🕒 ${DateTime.now().toIso8601String()}] Sync auto FLOTS & OPERATIONS');
  ...
  void startSlowSync() {
    ...
    debugPrint(
        '🐢⏰ Démarrage synchronisation lente PERSONNEL (intervalle: ${_slowSyncInterval.inMinutes} min)');
    debugPrint(
        '🔍 État: isAutoSyncEnabled=$_isAutoSyncEnabled, isOnline=$_isOnline, isSyncing=$_isSyncing');
    ...
    _slowSyncTimer = Timer.periodic(_slowSyncInterval, (timer) async {
      ...
      if (_isAutoSyncEnabled && !_isSyncing && _isOnline) {
        debugPrint(
            '🔄 [🕒 ${DateTime.now().toIso8601String()}] Sync lente PERSONNEL');
  ...
  Future<void> syncFlotsAndOperations() async {
    ...
    try {
      ...
      debugPrint(
          '✅ === SYNC FLOTS & OPERATIONS TERMINÉE: $successCount OK, $errorCount erreurs ===');
    } catch (e) {
  ...
  @Deprecated(
      'Utiliser TransferSyncService.syncTransfers() pour synchroniser les opérations')
  Future<bool> syncOperations() async {
    debugPrint(
        '⚠️ syncOperations() est obsolète - utilisez TransferSyncService.syncTransfers()');
  ...
  Future<void> queueOperation(Map<String, dynamic> operation,
    ...
    debugPrint(
        '📋 Opération mise en file d\'attente (total: $_pendingSyncCount, priorité: $priority)');
  }
  ...
  Future<void> _loadPendingOperations() async {
    try {
      ...
      if (pendingData != null && pendingData.isNotEmpty) {
        ...
        debugPrint(
            '📋 ${_pendingOperations.length} opération(s) en attente chargée(s) depuis le stockage');
      } else {
  ...
  Future<void> _loadPendingFlots() async {
    try {
      ...
      if (pendingData != null && pendingData.isNotEmpty) {
        ...
        debugPrint(
            '📪 ${_pendingFlots.length} flot(s) en attente chargé(s) depuis le stockage');
      } else {
  ...
  Future<void> syncPendingData() async {
    ...
    debugPrint(
        '🔄 Synchronisation de ${_pendingOperations.length} opérations en attente (triées par priorité)...');
    ...
    for (final operation in operationsToSync) {
      try {
        ...
        debugPrint(
            '📤 Upload opération: code_ops=${operation['code_ops']}, type=${operation['type']}, montant=${operation['montant_brut']}');
        debugPrint(
            '   Détails: agent_id=${operation['agent_id']}, shop_source_id=${operation['shop_source_id']}, client_id=${operation['client_id']}');
        debugPrint(
            '   Statut: ${operation['statut']}, Mode: ${operation['mode_paiement']}');
        ...
        final payload = {
          'entities': [operation],
          'user_id': operation['lastModifiedBy'] ??
              operation['last_modified_by'] ??
              'offline_user',
          'timestamp': DateTime.now().toIso8601String(),
        };
        ...
        if (response.statusCode == 200) {
          ...
          if (result['success'] == true) {
            ...
            debugPrint(
                '✅ Opération ${operation['code_ops']} synchronisée avec succès');
            ...
            try {
              ...
              if (codeOps != null && codeOps.isNotEmpty) {
                final localOp =
                    await LocalDB.instance.getOperationByCodeOps(codeOps);
                if (localOp != null) {
                  ...
                  debugPrint(
                      '💾 Opération code_ops=$codeOps marquée comme synchronisée dans LocalDB');
                } else {
                  debugPrint(
                      '⚠️ Opération code_ops=$codeOps non trouvée dans LocalDB');
                }
              }
            } catch (e) {
            ...
            debugPrint(
                '❌ Échec sync opération ${operation['code_ops']}: ${result['message']}');
            failedOperations.add(operation);
          }
        } else {
          debugPrint(
              '❌ Erreur HTTP ${response.statusCode} pour opération ${operation['code_ops']}');
    ...
    if (_pendingOperations.isEmpty) {
      ...
      await prefs.setString(
          'pending_operations', jsonEncode(_pendingOperations));
    }

    debugPrint(
        '✅ Synchronisation terminée: $synced réussies, ${failedOperations.length} échouées');
  ...
  Future<void> syncPendingFlots() async {
    ...
    debugPrint(
        '🔄 Synchronisation de ${_pendingFlots.length} flots en attente (triés par priorité)...');
          ...
          if (result['success'] == true) {
            ...
            try {
              ...
              if (reference != null && reference.isNotEmpty) {
                final localFlot =
                    await LocalDB.instance.getFlotByReference(reference);
                if (localFlot != null) {
                  ...
                  debugPrint(
                      '⚠️ Flot reference=$reference ignoré - Les FLOTs sont maintenant synchronisés via table operations');
                  ...
                  debugPrint(
                      '⚠️ Flot reference=$reference non trouvé dans LocalDB');
                }
              }
            } catch (e) {
    ...
    debugPrint(
        '✅ Synchronisation flots terminée: $synced réussies, ${failedFlots.length} échouées');
  ...
  /// DEPRECATED: Utiliser TransferSyncService.syncTransfers() à la place
  @Deprecated(
      'Utiliser TransferSyncService.syncTransfers() pour télécharger les opérations')
  Future<void> forceFullOperationsDownload({String? userId}) async {
    debugPrint(
        '⚠️ forceFullOperationsDownload() est obsolète - utilisez TransferSyncService.syncTransfers()');
    // Rediriger vers TransferSyncService
    try {
      final transferSyncService = TransferSyncService();
      await transferSyncService.syncTransfers();
  ...
  Future<void> forceResetTriangularDebtSync() async {
    try {
      debugPrint(
          '🔧 FORCE RESET triangular_debt_settlements sync - resetting circuit breaker');
      ...
      if (currentUser != null) {
        ...
        await _downloadTableData('triangular_debt_settlements',
            currentUser.id.toString(), currentUser.role);
        debugPrint('✅ triangular_debt_settlements sync completed successfully');
      } else {
        debugPrint(
            '❌ No current user found for triangular_debt_settlements sync');
        throw Exception('No authenticated user found');
      }
    } catch (e) {
      debugPrint(
          '❌ Error during force reset triangular_debt_settlements sync: $e');
      rethrow;
    }
  }
}
