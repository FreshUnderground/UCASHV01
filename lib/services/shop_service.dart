...
class ShopService extends ChangeNotifier {
  ...
  Future<void> loadShops(
    ...
    if (!forceRefresh && !clearBeforeLoad && _shops.isNotEmpty) {
      debugPrint(
          '✅ [ShopService] Utilisation du cache (${_shops.length} shops)');
      return;
    }
    ...
    try {
      ...
      if (forceRefresh && excludeShopId != null) {
        ...
        debugPrint(
            '✅ [ShopService] Shop ID $excludeShopId préservé dans le cache');
      ...
      if (clearBeforeLoad && _shops.isEmpty) {
        debugPrint(
            '⚠️ [ShopService] Aucun shop chargé après clearBeforeLoad - Vérifiez la synchronisation');
      }
  ...
  Future<bool> createShop({
    ...
    try {
      ...
      await _createDefaultCaisses(shopId, capitalCash, capitalAirtelMoney,
          capitalMPesa, capitalOrangeMoney);

      // Créer une clôture de la veille comme solde antérieur au lieu d'une opération de dépôt
      await _createInitialClosureAsAnterieur(shopId, capitalCash,
          capitalAirtelMoney, capitalMPesa, capitalOrangeMoney, designation);
  ...
  Future<Map<String, dynamic>?> updateShopViaAPI(ShopModel shop,
      {String userId = 'admin'}) async {
    try {
      debugPrint(
          '📤 [ShopService] Mise à jour du shop via API: ${shop.designation}');
      ...
      if (response.statusCode == 200) {
        ...
        if (result['success'] == true) {
          ...
          debugPrint(
              '👥 Agents affectés: ${result['affected_agents']['count']}');
  ...
  String getShopSourceDesignation(int? shopSourceId,
      {String? existingDesignation}) {
    return getShopDesignation(shopSourceId,
        existingDesignation: existingDesignation);
  }

  /// Obtenir la désignation du shop destination d'une opération ou d'un flot
  String getShopDestinationDesignation(int? shopDestinationId,
      {String? existingDesignation}) {
    return getShopDesignation(shopDestinationId,
        existingDesignation: existingDesignation);
  }

  // Créer les caisses par défaut pour un nouveau shop
  Future<void> _createDefaultCaisses(int shopId, double capitalCash,
      double capitalAirtel, double capitalMPesa, double capitalOrange) async {
    final caisseData = {
      'CASH': capitalCash,
  ...
  Map<String, dynamic> getShopsStats() {
    return {
      ...
      'averageCapital':
          _shops.isEmpty ? 0.0 : getTotalCapital() / _shops.length,
      'activeShops': _shops.where((shop) => shop.capitalActuel > 0).length,
    };
  }

  // Créer une clôture de la veille comme solde antérieur
  Future<void> _createInitialClosureAsAnterieur(
      int shopId,
      double capitalCash,
      double capitalAirtel,
      double capitalMPesa,
      double capitalOrange,
    ...
    try {
      ...
      final dateVeilleNormalisee =
          DateTime(dateVeille.year, dateVeille.month, dateVeille.day);
      ...
      final cloture = ClotureCaisseModel(
        ...
        notes:
            'Clôture initiale automatique lors de la création du shop $shopName - Servira de solde antérieur pour aujourd\'hui',
      );

      await LocalDB.instance.saveClotureCaisse(cloture);

      debugPrint(
          '✅ Clôture initiale créée pour $shopName - ${totalCapital.toStringAsFixed(2)} USD');
    } catch (e) {
    ...
    for (final shop in _shops) {
      if (shop.id != null) {
        ...
        final clotureExistante =
            await LocalDB.instance.getClotureCaisseByDate(shop.id!, dateVeille);
  ...
  Future<void> updateShopDirectly(ShopModel shop) async {
    try {
      ...
      if (index != -1) {
        _shops[index] = shop;
        debugPrint(
            '✅ [ShopService] Shop ${shop.designation} mis à jour directement (is_synced: ${shop.isSynced})');
      }
    } catch (e) {
  ...
  Future<void> _checkForDeletedShops() async {
    try {
      ...
      final response = await http
        ...
        onTimeout: () {
          throw Exception(
              'Timeout lors de la vérification des shops supprimés');
        },
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          ...
          if (deletedShops.isNotEmpty) {
            debugPrint(
                '🗑️ ${deletedShops.length} shop(s) supprimé(s) détecté(s) sur le serveur');
          ...
          debugPrint(
              '⚠️ Erreur lors de la vérification des shops supprimés: ${data['error']}');
        }
      } else {
        debugPrint(
            '⚠️ HTTP Error ${response.statusCode} lors de la vérification des shops supprimés');
      }
    } catch (e) {
  ...
  Future<void> _removeDeletedShopsLocally(List<int> deletedShopIds) async {
    try {
      debugPrint(
          '🗑️ Suppression locale de ${deletedShopIds.length} shop(s)...');
      ...
      debugPrint(
          '✅ Nettoyage local terminé: $removedCount shop(s) supprimé(s)');
    } catch (e) {
