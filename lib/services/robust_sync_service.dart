...
class RobustSyncService {
  ...
  final VirtualTransactionSyncService _virtualTransactionSync =
      VirtualTransactionSyncService();
  final CreditVirtuelSyncService _creditVirtuelSync =
      CreditVirtuelSyncService();
  ...
  Future<void> initialize() async {
    debugPrint(
        '🚀 ======== ROBUST SYNC SERVICE UNIFIÉ - INITIALISATION ========');
    ...
    if (_isEnabled && _isOnline) {
      ...
      debugPrint(
          '📊 Services intégrés: Transfer, VirtualTransaction, CreditVirtuel, Personnel, Deletion');
      debugPrint(
          '🚀 Optimisations actives: Cache multi-niveaux, Pagination, Circuit breaker adaptatif');
    } else {
  ...
  double _calculateErrorRate() {
    final totalOps = _fastSyncSuccessCount +
        _fastSyncErrorCount +
        _slowSyncSuccessCount +
        _slowSyncErrorCount;
  ...
  int _getPendingQueueSize() {
    return _modificationQueues.values
  ...
  Future<void> _processModification(Map<String, dynamic> modification) async {
    ...
    switch (type) {
      case 'create':
        await _syncService.uploadTableData(
            modification['table'], 'modification', 'system');
        break;
      case 'update':
        await _syncService.uploadTableData(
            modification['table'], 'modification', 'system');
  ...
  void _addModificationToQueue(Map<String, dynamic> modification,
      {int priority = 1}) {
  ...
  static Future<void> _performHeavySyncInIsolate(
  ...
  Future<Map<String, dynamic>?> _getDiskCache(
  ...
  /// Pagination intelligente avec pré-chargement
  Future<List<T>> loadPage<T>(String table, int page,
    ...
    if (preload) {
      ...
      _preloadPages<T>(table, page + 1, _preloadPagesCount)
          .catchError((e) => null);
    }
  ...
  void _startFastSyncTimer() {
    ...
    debugPrint(
        '⏰ Timer FAST SYNC démarré (${_fastSyncInterval.inMinutes} min)');
  }
  ...
  void _startSlowSyncTimer() {
    ...
    debugPrint(
        '⏰ Timer SLOW SYNC démarré (${_slowSyncInterval.inMinutes} min)');
  }
  ...
  Future<void> _performFastSync({bool isInitial = false}) async {
    ...
    debugPrint(
        '   Tables critiques: operations, flots, clients, comptes_speciaux, sims, virtual_transactions, retrait_virtuels, credit_virtuels, audit_log, reconciliations, triangular_debt_settlements');
    ...
    try {
      ...
      if (await _syncWithRetry('queue_depots_retraits', () async {
        debugPrint(
            '  💰 [PRIORITÉ 1] Sync dépôts/retraits via service spécialisé...');
      ...
      if (await _syncWithRetry('triangular_debt_settlements', () async {
        ...
        await _syncService.uploadTableData(
            'triangular_debt_settlements', 'auto_fast_sync', 'admin');
        debugPrint('  📥 Download RÈGLEMENTS TRIANGULAIRES...');
        await _syncService.downloadTableData(
            'triangular_debt_settlements', 'auto_fast_sync', 'admin');
      })) {
        successCount++;
      } else {
      ...
      if (await _syncWithRetry('flots', () async {
        ...
        debugPrint(
            '  ✅ FLOTs synchronisés via operations (type=flotShopToShop)');
      })) {
        successCount++;
      } else {
        errorCount++;
      ...
      if (await _syncWithRetry('comptes_speciaux', () async {
        ...
        await _syncService.uploadTableData(
            'comptes_speciaux', 'auto_fast_sync', 'admin');
      ...
      if (await _syncWithRetry('retrait_virtuels', () async {
        ...
        await _syncService.uploadTableData(
            'retrait_virtuels', 'auto_fast_sync', 'admin');
      ...
      if (await _syncWithRetry('audit_log', () async {
        ...
        await _syncService.uploadTableData(
            'audit_log', 'auto_fast_sync', 'admin');
      ...
      if (await _syncWithRetry('reconciliations', () async {
        ...
        await _syncService.uploadTableData(
            'reconciliations', 'auto_fast_sync', 'admin');
      ...
      // ========== ÉTAPE 9.2: SYNC CREDIT INTERSHOP TRACKING (CONSOLIDATION) ==========
      if (await _syncWithRetry('credit_intershop_tracking', () async {
        debugPrint('  🏦 Upload CREDIT_INTERSHOP_TRACKING...');
        await _syncService.uploadTableData(
            'credit_intershop_tracking', 'auto_fast_sync', 'admin');
        debugPrint('  📥 Download CREDIT_INTERSHOP_TRACKING...');
        await _syncService.downloadTableData(
            'credit_intershop_tracking', 'auto_fast_sync', 'admin');
      })) {
        successCount++;
        ...
        errorCount++;
        errors.add('credit_intershop_tracking');
      }

      // ========== ÉTAPE 9.3: SYNC SUPPRESSIONS ==========
      if (await _syncWithRetry('deletion_requests', () async {
        debugPrint('  🗑️ Upload DELETION_REQUESTS...');
        await _syncService.uploadTableData(
            'deletion_requests', 'auto_fast_sync', 'admin');
      ...
      debugPrint(
          '✅ FAST SYNC terminé en ${duration.inSeconds}s: $successCount OK, $errorCount erreurs');
      ...
      if (errors.isNotEmpty) {
        ...
        debugPrint(
            '🎉 FAST SYNC 100% réussi - Toutes les données critiques synchronisées !');
      }
    } catch (e, stack) {
  ...
  Future<void> _performSlowSync({bool isInitial = false}) async {
    ...
    debugPrint(
        '   Tables: commissions, cloture_caisse, shops, agents, document_headers');
    ...
    try {
      ...
      if (await _syncWithRetry('commissions', () async {
        ...
        await _syncService.uploadTableData(
            'commissions', 'auto_slow_sync', 'admin');
      ...
      if (await _syncWithRetry('cloture_caisse', () async {
        ...
        await _syncService.uploadTableData(
            'cloture_caisse', 'auto_slow_sync', 'admin');
      ...
      if (await _syncWithRetry('document_headers', () async {
        ...
        await _syncService.uploadTableData(
            'document_headers', 'auto_slow_sync', 'admin');
      ...
      final duration = DateTime.now().difference(startTime);
      debugPrint(
          '✅ SLOW SYNC terminé en ${duration.inSeconds}s: $successCount OK, $errorCount erreurs');
  ...
  Future<bool> _syncWithRetry(
      String tableName, Future<void> Function() syncFunction) async {
    ...
    while (attempt < maxRetries) {
      try {
        ...
        if (attempt < maxRetries) {
          ...
          debugPrint(
              '  ⚠️ $tableName échoué (tentative $attempt/$maxRetries), retry dans ${delay.inSeconds}s...');
  ...
  void _scheduleRetry(bool isFast) {
    ...
    debugPrint(
        '🔄 Retry programmé dans ${_retryDelay.inSeconds}s pour: ${tables.join(", ")}');
  ...
  void addModification(String table, String type, Map<String, dynamic> data,
      {int priority = 1}) {
    final modification = {
      'table': table,
      'type': type,
    ...
    debugPrint(
        '📝 Modification ajoutée à la queue: $type sur $table (priorité: $priority)');
  }
  ...
  Future<DeltaSyncResult> performDeltaOperationsSync({
    ...
    try {
      ...
      if (userId == null || userRole == null) {
        throw Exception(
            'Informations utilisateur manquantes pour la synchronisation');
      }
  ...
  Future<void> _processDeltaSyncResults(DeltaSyncResult result) async {
    try {
      ...
      if (result.newOperations.isNotEmpty) {
        debugPrint(
            '📥 Traitement de ${result.newOperations.length} nouvelles opérations');
      ...
      if (result.updatedOperations.isNotEmpty) {
        debugPrint(
            '🔄 Traitement de ${result.updatedOperations.length} opérations mises à jour');
