<?php
/**
 * Fix API endpoint path - Create admin_validate.php in correct location
 */
...
try {
    ...
    $expectedUrl = 'https://safdal.investee-group.com/server/api/sync/deletion_requests/admin_validate.php';
    ...
    $possiblePaths = [
        ...
        '/home/mahanaim/public_html/server/api/sync/deletion_requests/'
    ];
    
    echo "🔍 Checking possible API directory locations...\n";
    ...
    if (!$foundPath) {
        ...
        if (mkdir($createPath, 0755, true)) {
            ...
            $createPath = __DIR__ . '/../api/sync/deletion_requests/';
            if (mkdir($createPath, 0755, true)) {
                echo "✅ Created directory: $createPath\n";
                $foundPath = $createPath;
            } else {
                echo "❌ Failed to create directory structure\n";
                echo "⚠️ Manual action required: Create the directory structure manually\n";
                exit(1);
            }
        }
    }
    
    // Now copy the admin_validate.php file to the correct location
    $targetFile = $foundPath . 'admin_validate.php';
    
    echo "\n📄 Creating admin_validate.php at: $targetFile\n";
    
    // The admin_validate.php content (from user's message)
    $adminValidateContent = '<?php
/**
...
?>';
    ...
    if (file_put_contents($targetFile, $adminValidateContent)) {
        echo "✅ admin_validate.php created successfully\n";
        ...
        $testUrl = str_replace('/var/www/mahanaim.investee-group.com/', 'https://safdal.investee-group.com/', $targetFile);
        ...
        if ($httpCode === 404) {
            echo "❌ Still getting 404 - check web server configuration\n";
        } elseif ($httpCode === 400 || $httpCode === 200) {
            echo "✅ Endpoint is now accessible!\n";
        }
        
    } else {
        echo "❌ Failed to create admin_validate.php\n";
        echo "⚠️ Manual action required: Copy the file manually\n";
    }
    
    echo "\n" . str_repeat("=", 60) . "\n";
    echo "🎯 SOLUTION SUMMARY:\n";
    echo "1. ✅ Created API endpoint at: $targetFile\n";
    echo "2. ✅ File contains detailed logging for debugging\n";
    echo "3. ✅ Endpoint should now be accessible to the app\n";
    echo "\n";
    echo "🔄 NEXT STEPS:\n";
    echo "1. Test admin validation from the app again\n";
    echo "2. Check logs for [DELETION_REQUESTS] entries\n";
    echo "3. If still issues, check web server configuration\n";
    echo str_repeat("=", 60) . "\n";
