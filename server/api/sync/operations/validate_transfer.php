...
try {
    ...
    if (!$hasReference && !$hasId && !$hasComposite) {
        throw new Exception(
            'Identification insuffisante. Fournissez soit:\n' .
            '1. reference (si disponible)\n' .
            '2. operation_id (si connu)\n' .
            '3. Combinaison: shop_source_id + date_operation + montant_net + destinataire'
        );
    }
    ...
    // Strategy 2: Par ID (si fourni)
    // Strategy 3: Par combinaison unique (shop_source + date + montant + destinataire)
    ...
    if (!$operation && $hasId) {
        // Strategy 2: Recherche par ID (moins fiable)
        $stmt = $db->prepare("
            SELECT * FROM operations 
            WHERE id = :id
            FOR UPDATE
    ...
    if (!$operation && $hasComposite) {
        // Strategy 3: Recherche par combinaison unique (le plus fiable!)
        $stmt = $db->prepare("
            SELECT * FROM operations 
    ...
    $finalStmt = $db->prepare("SELECT * FROM operations WHERE id = :id");
    $finalStmt->execute([':id' => $operationId]);
