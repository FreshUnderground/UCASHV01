    ...
    foreach ($entities as $entity) {
        try {
            ...
            if ($exists) {
                ...
                $updateStmt->execute([
                    ':id' => $entity['id'],
                    ':devise_source' => $entity['devise_source'] ?? 'USD',
                    ':devise_cible' => $entity['devise_cible'] ?? '',
                    ':type' => $entity['type'] ?? 'MOYEN',
                    ':taux' => $entity['taux'] ?? 0,
                    ':est_actif' => $entity['est_actif'] ?? 1,
                    ':date_effet' => $entity['date_effet'] ?? date('Y-m-d H:i:s'),
                ...
                $syncStmt = $pdo->prepare("UPDATE taux SET is_synced = 1, synced_at = :synced_at WHERE id = :id");
                $syncStmt->execute([
                    ':id' => $entity['id'],
                    ':synced_at' => $syncedAt
                ]);
                ...
                // Insertion d'un nouveau taux
                $insertStmt = $pdo->prepare("
                    INSERT IGNORE INTO taux (
                        devise_source, devise_cible, type, taux, est_actif, date_effet,
