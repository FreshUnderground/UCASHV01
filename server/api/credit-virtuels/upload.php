<?php
/**
 * API pour uploader les crédits virtuels vers le serveur
...
try {
    ...
    foreach ($credits as $creditData) {
        try {
            ...
            $checkSql = "SELECT id FROM credit_virtuels WHERE id = :id";
            $checkStmt = $conn->prepare($checkSql);
            $checkStmt->bindParam(':id', $creditData['id'], PDO::PARAM_INT);
            $checkStmt->execute();
            $existingCredit = $checkStmt->fetch();
            ...
            if ($existingCredit) {
                ...
                $stmt->bindParam(':id', $creditData['id'], PDO::PARAM_INT);
            } else {
                // Insertion
                $sql = "
            ...
            $stmt->bindParam(':devise', $creditData['devise'] ?? 'USD', PDO::PARAM_STR);
            ...
            $stmt->bindParam(':type_beneficiaire', $creditData['type_beneficiaire'] ?? 'shop', PDO::PARAM_STR);
            ...
            $stmt->bindParam(':statut', $creditData['statut'], PDO::PARAM_STR);
            ...
            $stmt->bindParam(':montant_paye', $creditData['montant_paye'] ?? 0.0, PDO::PARAM_STR);
            ...
            $stmt->bindParam(':last_modified_at', $creditData['last_modified_at'], PDO::PARAM_STR);
            $stmt->bindParam(':last_modified_by', $creditData['last_modified_by'], PDO::PARAM_STR);

            $stmt->execute();

            // Récupérer l'ID du crédit (nouveau ou existant)
            $serverId = $existingCredit ? $creditData['id'] : $conn->lastInsertId();

            $syncedCredits[] = [
                'id' => $creditData['id'], // ID local
                'server_id' => (int)$serverId, // ID serveur
                'status' => $existingCredit ? 'updated' : 'created'
            ];
    ...
    if (empty($errors) || count($syncedCredits) > 0) {
        $conn->commit();
        
        // Log de l'activité
        error_log("API credit-virtuels/upload: Shop $shopId - " . count($syncedCredits) . " crédits synchronisés, " . count($errors) . " erreurs");
        ...
        echo json_encode([
            ...
            'errors' => $errors,
            'total_synced' => count($syncedCredits),
    ...
    error_log("Erreur PDO dans credit-virtuels/upload: " . $e->getMessage());
    ...
    error_log("Erreur dans credit-virtuels/upload: " . $e->getMessage());
}
?>
